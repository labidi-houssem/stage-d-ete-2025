...
export default function AdminCandidatesCVList() {
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-4xl font-bold bg-gradient-to-r from-red-700 to-rose-600 bg-clip-text text-transparent">
              Gestion des CVs - Admin
            </h1>
          </div>
          <p className="text-gray-700 text-lg">Gérez et consultez tous les CVs des candidats</p>
        </div>
          ...
          <div className="bg-white/70 backdrop-blur-sm rounded-2xl shadow-xl border border-white/20 p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {candidates.length > 0 ? Math.round(candidates.reduce((acc, c) => acc + c.cv.completionPercentage, 0) / candidates.length) : 0}%
                </p>
              </div>
            </div>
          </div>
              ...
              <div key={candidate.id} className="bg-white/70 backdrop-blur-sm rounded-2xl shadow-xl border border-white/20 p-6 hover:shadow-2xl transition-all duration-300">
                <div className="flex items-start justify-between mb-4">
                  ...
                  <span className={`px-3 py-1 rounded-full text-sm font-medium ${getCompletionColor(candidate.cv.completionPercentage)}`}>
                    {candidate.cv.completionPercentage}%
                  </span>
                </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="text-sm text-gray-500">
                    <p>Inscrit le {new Date(candidate.createdAt).toLocaleDateString('fr-FR')}</p>
                  </div>
                  <button
                    onClick={() => router.push(`/admin/candidate-cv/${candidate.id}`)}
